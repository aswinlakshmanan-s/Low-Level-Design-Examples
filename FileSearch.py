'''You're building your own find command like in Linux:

You give it a starting folder (root),

You add filters (e.g., "files bigger than 5MB", or "only .txt files"),

It searches through the whole folder structure,

It prints matching files either if:

any filter matches (OR filtering),

all filters match (AND filtering).'''
